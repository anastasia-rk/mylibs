...
g = [0.87 0.87 0.87];
plot(axs,x'*radius,y'*radius,'-','color',g,'LineWidth',1);                       % Draw dooted circle lines
plot(axs,x*radiusmax,y*radiusmax,'-','color',g,'LineWidth',1);                   % Redraw last circle line in solid style
...
plot(axs,R*cosd(axisangles),R*sind(axisangles),'-','color',g,'LineWidth',1);     % Draw radial axis, in the specified angles
...
cs = [-cst; cst];
sn = [-snt; snt];
line(rmax * cs, rmax * sn, 'Color', g, 'LineWidth', 1, ...
...
elseif LegendType==1                                                       % If legend type is colorbar
    ...
    colorbar('YTick',vwinds,'Location','southoutside');                                              % The values shown in the colorbar are the intenisites.
end
